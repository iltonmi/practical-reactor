...
//                numberService2().startWith(numberService1());
    ...
    public void mail_box_switcher() {
        //todo: feel free to change code as you need
        Flux<Message> myMail = mailBoxPrimary().any(m -> m.metaData.equals("spam"))
                        .flatMapMany(flag -> flag ? mailBoxSecondary() : mailBoxPrimary());
    ...
    public Mono<String> chooseSource() {
        return Mono.defer(() -> {
            if (Objects.equals(sourceRef.get(), "A")) return sourceA(); //<- choose if sourceRef == "A"
            else if (Objects.equals(sourceRef.get(), "B")) return sourceB(); //<- choose if sourceRef == "B"
            else return Mono.empty(); //otherwise, return empty
        });
    }
